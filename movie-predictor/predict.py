...
def build_feature_row(inputs: dict, feature_cols: list[str]) -> pd.DataFrame:
    ...
    # RT scores
    row["rt_score_prerelease"] = inputs.get("rt_score", 0) or 0
    row["rt_reviews_prerelease"] = inputs.get("rt_reviews", 0) or 0
    row["rt_audience_prerelease"] = inputs.get("rt_audience", 0) or 0
...
def main():
    ...
    if os.path.exists("data/movies_merged.csv"):
        ...
        if rt:
            similar = df[
                (df["rt_score_prerelease"].between(rt - 8, rt + 8)) &
                (df["opening_weekend"].notna())
            ].nlargest(5, "opening_weekend")
            if not similar.empty:
                print("\n  Similar RT-scored films (for context):")
                for _, row in similar.iterrows():
                    print(f"    {row['title']:35s} RT={row.get('rt_score_prerelease', '?'):>3}%  "
