...
Features used:
  - rt_score_release_day      (tomatometer at release)
  - rt_reviews_release_day    (review count proxy = press coverage)
  - rt_audience_release_day   (audience score at release)
...
BASE_FEATURES = [
    "rt_score_release_day",
    "rt_reviews_release_day",
...
def fill_missing_features(df: pd.DataFrame, feature_cols: list[str]) -> pd.DataFrame:
    ...
    for col in feature_cols:
        if col not in df.columns:
            df[col] = 0
        if col in ["rt_score_release_day", "rt_score_final", "rt_audience_release_day"]:
            # Fill with median — missing RT score is informative (limited release / no buzz)
            median_val = df[col].median()
            df[col] = df[col].fillna(median_val)
        elif col == "rt_reviews_release_day":
...
def main():
    ...
    has_rt = df["rt_score_release_day"].notna() | df["rt_score_final"].notna()
    ...
    df_model["rt_score_release_day"] = df_model["rt_score_release_day"].fillna(
        df_model["rt_score_final"]
    )
    df_model["rt_audience_release_day"] = df_model["rt_audience_release_day"].fillna(
    ...
    worst = df_model.nlargest(10, "abs_error_M")[
        ["title", "year", "actual", "oof_pred", "error_pct", "rt_score_release_day"]
    ].copy()
    worst["actual_M"] = worst["actual"] / 1e6
    worst["pred_M"] = worst["oof_pred"] / 1e6
    print(worst[["title", "year", "actual_M", "pred_M", "error_pct", "rt_score_release_day"]].to_string())
